...
class SequentialCIFAR100(IncrementalDataset):
    ...
    def get_data_loaders(self):
        ...
        train_dataset = MyCIFAR100(base_path() + 'CIFAR10', train=True, transform=self.cifar10_train_transform)
        if self.args.validation:
            train_dataset, test_dataset = get_train_val(train_dataset,
                                                        self.cifar10_test_transform, self.NAME)
        else:
            test_dataset = MyCIFAR100(base_path() + 'CIFAR10', train=False, transform=self.cifar10_test_transform)
