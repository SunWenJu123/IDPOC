...
from datasets.utils.continual_dataset import IncrementalDataset, store_masked_loaders, getfeature_loader
...
class SequentialTinyImagenet(IncrementalDataset):
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        if self.args.transform == 'pytorch':
            ...
            self.tinyimg_train_transform = transforms.Compose([
                transforms.Resize(256),
                transforms.CenterCrop(224),
                transforms.ToTensor(),
                self.normalization_transform])
            self.tinyimg_test_transform = transforms.Compose([
            ...
            self.tinyimg_train_transform = transforms.Compose([
                ...
                transforms.ToTensor()])
            self.tinyimg_test_transform = transforms.Compose([transforms.ToTensor()])


    def get_data_loaders(self):
        train_dataset = MyTinyImagenet(base_path() + 'TINYIMG',
                                 train=True, download=True, transform=self.tinyimg_train_transform)
        if self.args.validation:
            train_dataset, test_dataset = get_train_val(train_dataset,
                                                    self.tinyimg_test_transform, self.NAME)
        else:
            test_dataset = TinyImagenet(base_path() + 'TINYIMG',
                        train=False, download=True, transform=self.tinyimg_test_transform)
